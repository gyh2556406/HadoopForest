...
import sequencrFile.PathFilters;
...
public final class DFUtils {
   ...
   */
  public static void writeArray(DataOutput out, TreeNode[] array) throws IOException {
    out.writeInt(array.length);
    for (TreeNode w : array) {
      w.write(out);
    }
  }
   ...
   * @throws java.io.IOException
   */
  public static TreeNode[] readNodeArray(DataInput in) throws IOException {
      log.info("Start to readNodeArray");
      int length = in.readInt();
      log.info("read length done: "+length);

      TreeNode[] nodes = new TreeNode[length];
      for (int index = 0; index < length; index++) {
          nodes[index] = new TreeNode();
          nodes[index] = TreeNode.read(in);
      }
  ...
  public static double[] readDoubleArray(DataInput in) throws IOException {
      log.info("Start to readDoubleArray");
    int length = in.readInt();
      log.info("read length done: "+length);
  ...
  public static void storeWritable(Configuration conf, Path path, RandomForest randomForest) throws IOException {
    FileSystem fs = path.getFileSystem(conf);

    try (FSDataOutputStream out = fs.create(path)) {
        randomForest.write(out);
    }
  }
   ...
   * @throws java.io.IOException if things go poorly
   */
